...
import RoomActionModal from '../components/common/RoomActionModal';
import { useAuth } from '../contexts/AuthContext';
import { useNavigate } from 'react-router-dom';
import DrawingLogo from '../components/landing/DrawingLogo';
import RoomFullModal from '../components/landing/RoomFullModal';
...
const LandingPage: React.FC = () => {
  ...
  const [roomActionType, setRoomActionType] = useState<'create' | 'join'>('create');
  ...
  // 모달 확인 버튼 핸들러
  const handleRoomActionConfirm = (inputValue: string) => {
    if (roomActionType === 'create') {
      handleCreateRoomWithTitle(inputValue);
    } else {
  ...
  // 제목으로 방 만들기 함수
  const handleCreateRoomWithTitle = async (title: string) => {
    try {
      if (!title) {
        return;
      }
    ...
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {showRoomActionModal && (
        <RoomActionModal
          ...
          closeModal={() => setShowRoomActionModal(false)}
          modalType={roomActionType}
          onConfirm={handleRoomActionConfirm}
        />
      )}
    </div>
  );
